...
def run_sanity_check(test_dir):
    ...
    print('Please enter the path to the file that contains your test '
    ...
    test_function_names = \
        list(filter(lambda x: inspect.
                    isfunction(getattr(module, x)) and not x.startswith('__'),
                    dir(module)))

    test_functions_for_get = list(
        filter(lambda x: inspect.getsource(getattr(module, x)).
    ...
    if not test_functions_for_get:
        ...
        WARNING_COUNT += 1
        print(FAIL_COLOR + "No test cases were detected for the GET() "
                           "method.")
        print(FAIL_COLOR + "\nPlease make sure you have a test case "
                           "for the GET method.\
            This MUST test both the status code as "
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_CODE:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR + "Your test case for GET() "
                               "does not seem to be testing "
                               "the response code.\n")

        if not TEST_FOR_GET_METHOD_RESPONSE_BODY:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR + "Your test case for GET() does "
                               "not seem to be testing the "
    ...
    if not test_functions_for_post:
        ...
        if len(test_functions_for_post) == 1:
            ...
            print(FAIL_COLOR + "Only one test case was detected "
                               "for the POST() method.")
            print(FAIL_COLOR + "Please make sure you have two "
        ...
        if len(test_functions_for_post) >= 2 and \
            ...
            WARNING_COUNT += 1
            print(
                FAIL_COLOR + "You do not seem to have TWO separate test "
                             "cases, one for each possible prediction "
                             "that your model can make.")

    SANITY_TEST_PASSING = \
        SANITY_TEST_PASSING and TEST_FOR_GET_METHOD_RESPONSE_CODE and \
    ...
    print(WARN_COLOR + "This is a heuristic based sanity testing "
                       "and cannot guarantee the correctness "
                       "of your code.")
    print(WARN_COLOR + "You should still check your work against "
